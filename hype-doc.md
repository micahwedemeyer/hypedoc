...
- **Volunteered to lead oncall unification for a newly merged 20-person team formed from 5 former sub-teams, navigating competing opinions and interpersonal friction to deliver a stable pager rotation under extreme time pressure.** When layoffs collapsed five separate teams — each with their own services, PagerDuty setups, escalation policies, and oncall cultures — into a single Payment Acquiring team, there was no unified oncall and no clear owner for fixing it. Stepped up immediately and drove the effort end-to-end, despite having no mandate or formal authority. The challenge was as much interpersonal as technical: engineers had strong, conflicting opinions about rotation frequency (6 weeks vs. 20 weeks), domain expertise requirements, timezone fairness for APAC/CET engineers, and whether generalists could safely handle services like PBS stale batches where mistakes risk double-capturing millions of dollars. Managed the process by creating structured forums for input, ensuring every engineer's concerns were documented and visibly addressed in the plan, and finding pragmatic compromises — like permanent daytime layers for globally distributed engineers and a round-robin escalation model with zero obligations on backup, which addressed burnout fears without sacrificing coverage.

  **Stage 1 — Stabilization ([plan](https://docs.google.com/document/d/1HYrkBAzbAEZDsWeX-Erl4Jv-0O1TPGJEL19K_GCaYRQ/edit), shipped March 2026):** Designed and implemented a 3-rotation domain-aligned model (Afterpay Platform, Payments Core & Global Cards, Regional Gateways & APMs) that got the team to a stable baseline within days. Key design decisions: on-call engineers do no sprint work, Monday-to-Monday handoffs at 1200 ET, 5-level escalation chains with round-robin secondary (no laptop requirement), and permanent daytime pagers for APAC and CET engineers. Managed the implementation complexity of synchronizing dual PagerDuty schedules across legacy escalation policies that couldn't yet be consolidated in Terraform. Achieved team-wide consensus — every engineer signed off with "I don't hate this" or better — a meaningful outcome given the emotional context of post-layoff uncertainty.

  **Stage 2 — Unification ([plan](https://docs.google.com/document/d/1CmO48uUUNP4HmBrXmoWILfZ0LT7jkDvk9I71b0Nwubo/edit), drafted April 2026):** After a month of hands-on experience validated Stage 1, authored the unification proposal to merge from 3 rotations down to 2 (and ultimately 1 for all Square services). Systematically analyzed every objection: "too dangerous for a noob" (PBS stale batches — addressed via runbook improvements and acquiring-oncall skill guards), mandate-driven paging spikes (addressed via expert scheduling around known dates), follow-the-sun sustainability (trialed with opt-out safety valve), and burnout risk (mitigated by the no-sprint-work policy reducing individual frequency from every 6 weeks to every 12). Framed each concern with concrete data — only 2 escalations in the prior month, zero real SEVs — to build confidence that unification was safe. The proposal reduced primary oncall frequency from every 6–7 weeks to every 12 weeks per engineer while expanding coverage to 59 services under a single rotation.
